...
use crate::{
    normalize_logical_path, path_is_file, remove_empty_dirs_recursive, resource_manifest_filename,
    resource_manifest_url, vfs_path, PathOutcomeTracker, ProgressLane, ProgressSender,
    ResourceManifestKind, RESOURCE_GROUP_BASE, RESOURCE_GROUP_MAIN,
};
use griffr_core::build_cdn_file_url;
use griffr_hypergryph_api::client::ApiClient;
use griffr_hypergryph_api::crypto::RES_INDEX_KEY;
use griffr_hypergryph_api::protocol::DEFAULT_PLATFORM;
use griffr_hypergryph_api::ApiTarget;
...
impl VfsManifestCommit {
    pub fn claim(&self) -> crate::ArtifactClaim {
        crate::ArtifactClaim::new(
            self.dest.clone(),
            crate::ArtifactExpectation::new(
                &self.logical_path,
                &self.expected_md5,
...
pub struct VfsTaskPlan {
    pub tasks: Vec<Task>,
    pub claims: Vec<crate::ArtifactClaim>,
...
pub fn commit_vfs_manifests(manifests: &[VfsManifestCommit]) -> Result<()> {
    for manifest in manifests {
        crate::task_pool::fs_ops::write_atomic_bytes(&manifest.dest, &manifest.encrypted_bytes)?;
    }
    Ok(())
}
...
impl PersistentManagedFile {
    fn expectation(&self) -> crate::ArtifactExpectation {
        crate::ArtifactExpectation::new(&self.path, &self.md5, Some(self.size))
    }
}
...
async fn read_local_res_index_document(
    path: &Path,
) -> Result<Option<griffr_hypergryph_api::client::ResIndexDocument>> {
    ...
    let decrypted =
        griffr_hypergryph_api::crypto::decrypt_res_index(encrypted_b64.trim(), RES_INDEX_KEY)
            .map_err(|e| Error::Message {
                context: "VFS error: ",
                detail: format!("Failed to decrypt {}: {e}", path.display()),
            })?;
    let index =
        serde_json::from_str::<griffr_hypergryph_api::ResIndex>(&decrypted).map_err(|e| {
            Error::Message {
                context: "VFS error: ",
                detail: format!("Failed to parse {}: {e}", path.display()),
            }
        })?;
    let md5 = griffr_core::to_hex(&md5::Md5::digest(&encrypted_bytes));
    Ok(Some(griffr_hypergryph_api::client::ResIndexDocument {
        index,
...
fn res_index_to_ensure_tasks(
    index: &griffr_hypergryph_api::ResIndex,
    ...
    for file in &index.files {
        ...
        let relative = crate::task_pool::fs_ops::path_safety::parse_safe_relative_path(
            "resource index path",
            &file.name,
        )?;
...
fn write_persistent_state(path: &Path, state: &PersistentVfsState) -> Result<()> {
    ...
    crate::task_pool::fs_ops::write_atomic_bytes(path, &bytes)
}
...
async fn prune_previous_managed_files(
    ...
    let prune_jobs = stream::iter(jobs)
        .map(|(path, expectation)| async move {
            crate::compat_fs::run_blocking("Persistent VFS prune", move || {
                ...
                if crate::task_pool::fs_ops::verify_artifact(
                    &path,
                    &expectation,
                    crate::ArtifactSource::Existing,
                )
                .is_err()
                {
                    return Ok(false);
...
mod tests {
    use super::*;
    use griffr_hypergryph_api::{ResIndex, ResIndexFile};
    ...
    async fn prune_removes_only_unchanged_previous_managed_files() {
        ...
        let expected_md5 = griffr_core::to_hex(&md5::Md5::digest(b"same"));
