...
use crate::artifact::physical_path_key;
use crate::task_pool::{
    FileEnsureTask, Task, TaskOutcome, TaskPoolRunner, TaskProgress, TransferClass,
};
use crate::{
    resource_manifest_filename, resource_manifest_url, ArtifactClaim, ArtifactExpectation,
    PathOutcomeTracker, ProgressLane, ProgressSender, ResourceManifestKind,
};
    ...
    while let Some((resource, document)) = resource_documents.try_next().await? {
        ...
        if new_manifest && !options.allow_repair {
            tasks.push(Task::Verify {
                ...
                expected_hash: crate::ContentHash::from(&manifest_commit.expected_md5),
