...
use crate::{
    ...
    normalize_logical_path, LocalInstall,
};
use griffr_core::GameId;
use griffr_hypergryph_api::{GameFileEntry, GAME_FILES_NAME};
...
pub async fn read_local_game_files(install_path: &Path) -> Result<Option<Vec<GameFileEntry>>> {
    ...
    Ok(griffr_hypergryph_api::client::parse_game_files_owned(encrypted).map(Some)?)
}
...
async fn remove_verified_obsolete_game_files(
    ...
    task_pool_runner: &mut crate::task_pool::TaskPoolRunner,
    ...
    let cleanup_paths = obsolete
        ...
        .collect::<Vec<_>>();
    crate::compat_fs::run_blocking("obsolete cleanup path check", move || {
    ...
    let tasks = obsolete
        .iter()
        .map(|(entry, relative)| crate::task_pool::Task::Verify {
        ...
        .collect::<Vec<_>>();
    let task_progress = crate::task_pool::TaskProgress::new(crate::ProgressSender::disabled())
        .with_verify(crate::ProgressLane::FILE_ENSURE_VERIFY, tasks.len());
    ...
    let verified = result
        ...
        .filter_map(|outcome| match outcome {
            crate::task_pool::TaskOutcome::Verified { path, ok: true, .. } => {
...
pub async fn remove_blocking_obsolete_game_files(
    ...
    for relative in owned_directories {
        crate::remove_empty_dir(install_path.join(relative)).await?;
    }
...
mod tests {
    ...
    async fn obsolete_cleanup_removes_unchanged_and_retains_modified_files() {
        ...
        let mut runner = crate::task_pool::TaskPoolRunner::new(
            crate::task_pool::TaskPoolConfig::for_file_ensure(),
        )
        .unwrap();
