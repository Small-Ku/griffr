...
use crate::{
    griffr_archives_path, is_griffr_private_path, is_launcher_metadata_path,
...
pub async fn ensure_game_files_from_manifest_with_pool(
    ...
    task_pool_runner: Option<&mut crate::task_pool::TaskPoolRunner>,
...
async fn ensure_game_files_impl(
    ...
    for (entry, relative) in planned_entries {
        ...
        if config.dry_run {
            let destination_matches = if metadata_only {
                crate::task_pool::verify::build_metadata_issue(
                    &install_path.join(&relative),
                    &entry.path,
                    crate::ContentHash::from(&entry.md5),
                    entry.size,
                )
                .is_none()
            } else {
                crate::task_pool::verify::build_issue(
                    &install_path.join(&relative),
                    &entry.path,
                    crate::ContentHash::from(&entry.md5),
                    Some(entry.size),
    ...
    let total = tasks.len();
    let task_progress = crate::task_pool::TaskProgress::new(progress)
    ...
    let run_result = if let Some(runner) = task_pool_runner {
        ...
        let pool_cfg = crate::task_pool::TaskPoolConfig::for_file_ensure();
        crate::task_pool::run_tasks_with_progress(tasks, pool_cfg, task_progress)
    };
    ...
    for event in result.outcomes {
        match event {
            ...
            crate::task_pool::TaskOutcome::Downloaded { path, bytes } => {
                outcomes.record_downloaded(&path, bytes);
            }
            crate::task_pool::TaskOutcome::Failed { path, reason } => {
