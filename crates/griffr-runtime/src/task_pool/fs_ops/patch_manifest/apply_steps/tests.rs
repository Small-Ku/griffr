use super::*;
use crate::task_pool::verify::{file_md5, VerifiedArtifactCache};
use crate::{griffr_patch_path, PlannedPatchEntry, PlannedPatchSource};
use md5::Digest;
...
fn patch_apply_defers_version_marker_and_preserves_final_output() {
    ...
    assert_eq!(proofs[0].source(), crate::ArtifactSource::Existing);
...
fn corrupt_local_payload_does_not_replace_existing_destination() {
    ...
    let entry = PlannedPatchEntry {
        ...
        expected_md5: griffr_core::to_hex(&md5::Md5::digest(b"expected payload")),
        expected_size: 16,
        source: PlannedPatchSource::Local {
