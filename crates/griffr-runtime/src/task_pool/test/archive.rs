...
fn full_archive_commits_verified_entries_and_skips_owned_paths() {
    ...
    let source_dir = crate::griffr_archives_path(tmp.path());
    ...
    let expected = archive_expected_files(vec![
        griffr_hypergryph_api::GameFileEntry {
            path: "archive.bin".to_string(),
            md5: griffr_core::to_hex(&Md5::digest(b"archive output")),
            size: 14,
        },
        griffr_hypergryph_api::GameFileEntry {
    ...
    let tasks = vec![Task::OpenArchive {
        ...
        password: None,
        patch_options: crate::PatchApplyOptions::default(),
...
fn full_archive_applies_deferred_delete_after_payload_commits() {
    ...
    let tasks = vec![Task::OpenArchive {
        ...
        password: None,
        patch_options: crate::PatchApplyOptions::default(),
        expected_files: archive_expected_files(vec![griffr_hypergryph_api::GameFileEntry {
            path: "payload.bin".to_string(),
            md5: griffr_core::to_hex(&Md5::digest(b"new payload")),
            size: 11,
        }]),
...
fn full_archive_keeps_earlier_commits_when_a_later_entry_fails() {
    ...
    let expected = archive_expected_files(vec![
        griffr_hypergryph_api::GameFileEntry {
            ...
            md5: griffr_core::to_hex(&Md5::digest(b"new first")),
            size: 9,
        },
        griffr_hypergryph_api::GameFileEntry {
...
fn extract_task_spawns_vfs_patch_and_delete_manifest_follow_up_tasks() {
  ...
  "files": [
    ...
    let tasks = vec![Task::OpenArchive {
        ...
        password: None,
        patch_options: crate::PatchApplyOptions::default(),
        expected_files: crate::task_pool::archive_expected_files(Vec::new()),
        excluded_commit_paths: std::sync::Arc::new(std::collections::BTreeSet::new()),
    }];

    let (progress_sender, progress_receiver) = crate::ProgressSender::channel();
    let progress = TaskProgress::new(progress_sender)
        .with_commit(crate::ProgressLane::ARCHIVE_COMMIT)
        .with_patch(crate::ProgressLane::ARCHIVE_PATCH)
        .with_delete(crate::ProgressLane::ARCHIVE_DELETE);
    ...
    assert!(progress_updates.iter().any(|update| matches!(
        update,
        crate::ProgressUpdate::Advanced {
            lane: crate::ProgressLane::ARCHIVE_COMMIT,
            finished,
            total: Some(total),
            ..
    ...
    assert!(progress_updates.iter().any(|update| matches!(
        update,
        crate::ProgressUpdate::Advanced {
            lane: crate::ProgressLane::ARCHIVE_PATCH,
            finished: 1,
            total: Some(1),
            ..
        }
    ...
    assert!(progress_updates.iter().any(|update| matches!(
        update,
        crate::ProgressUpdate::Advanced {
            lane: crate::ProgressLane::ARCHIVE_DELETE,
            finished: 1,
            total: Some(1),
            ..
        }
...
fn patch_archive_extracts_only_selected_payloads_and_direct_entries() {
    ...
    zip.start_file("patch.json", FileOptions::<()>::default())
        .unwrap();
    let existing_md5 = griffr_core::to_hex(&Md5::digest(b"already correct"));
    zip.write_all(
        format!(
            r#"{{
  "version": "75.0.0",
  ...
  "files": [
    ...
    let tasks = vec![Task::OpenArchive {
        ...
        expected_files: archive_expected_files(vec![griffr_hypergryph_api::GameFileEntry {
            ...
            md5: griffr_core::to_hex(&Md5::digest(b"updated payload")),
            size: 15,
        }]),
        excluded_commit_paths: std::sync::Arc::new(std::collections::BTreeSet::from([
            "task-owned.bin".to_string(),
