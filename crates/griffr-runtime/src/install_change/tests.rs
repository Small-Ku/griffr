...
use super::*;
use crate::task_pool::fs_ops::{commit_observed_artifact, verify_artifact, write_atomic_bytes};
use crate::{
...
fn install_change_process_kill_worker() {
    ...
    match scenario.as_str() {
        ...
        "config_before_finish" => {
            ...
            crate::test_checkpoint::hit("change.before_finish");
            let requested = state(
                InstallChangeKind::Update,
                InstallChangeSource::FullArchive,
                Some("1.0"),
        ...
        "finish_change" => {
            ...
            finish_install_change(&root, &requested).unwrap();
            crate::test_checkpoint::hit("change.after_finish");
        }
