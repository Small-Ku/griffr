...
use crate::task_pool::fs_ops::{path_safety::parse_safe_relative_path, write_atomic_bytes};
use crate::ContentHash;
use griffr_core::RegionId;
use griffr_yostar_api::{yostar_arknights_target, yostar_region_from_game_tag};
use griffr_yostar_api::{YostarGameConfig, YostarManifest, YostarManifestEntry};
...
pub fn validate_remote_yostar_manifest(manifest: &YostarManifest) -> Result<()> {
    ...
    for entry in &manifest.files {
        ...
        let normalized = crate::normalize_logical_path(&entry.path);
