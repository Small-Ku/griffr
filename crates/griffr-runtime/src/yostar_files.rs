...
use crate::files::reuse::FileEnsureSummary;
use crate::task_pool::{
...
use griffr_yostar_api::{YostarManifest, YostarManifestEntry};

fn planned_entries(manifest: &YostarManifest) -> Result<Vec<(&YostarManifestEntry, PathBuf)>> {
    crate::validate_remote_yostar_manifest(manifest)?;
...
fn summarize_result(
    ...
    graph: &crate::task_pool::TaskGraphSummary,
...
pub async fn cleanup_yostar_obsolete_files(
    ...
    for entry in &current.files {
        ...
        match crate::task_pool::verify::build_issue(&path, &entry.path, &expected, Some(entry.size))
        {
            None => {
            ...
            Some(issue) if issue.kind == crate::FileIssueKind::Missing => {}
    ...
    for relative in dirs {
        let _ = crate::remove_empty_dir(install_path.join(relative)).await;
    }
    Ok(summary)
}
...
mod tests {
    use super::*;
    use crate::task_pool::{TaskPoolConfig, TaskPoolRunner};
    use griffr_yostar_api::YostarManifestEntry;
    ...
    fn launch_metadata_check_catches_missing_or_wrong_size_without_hashing() {
        compio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            assert_eq!(summary.issues[0].kind, crate::FileIssueKind::SizeMismatch);
        });
    }
    ...
    fn normal_update_trusts_same_manifest_size_but_full_verify_hashes() {
        compio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            .unwrap();
            assert_eq!(summary.issues.len(), 1);
            assert_eq!(summary.issues[0].kind, crate::FileIssueKind::HashMismatch);
        });
    }
