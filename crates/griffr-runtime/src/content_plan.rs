...
use crate::{is_griffr_private_path, normalize_logical_path, ArtifactClaim, ArtifactExpectation};
use compio::bytes::Bytes;
use griffr_hypergryph_api::client::ApiClient;
use griffr_hypergryph_api::ApiTarget;
use griffr_hypergryph_api::{GameFileEntry, GetLatestGameResponse, PackageInfo};
...
impl ContentPlan {
    pub fn from_snapshot(
        ...
        for entry in &snapshot.entries {
            ...
            let domain = if crate::is_launcher_metadata_path(&entry.path) {
        ...
        for claim in resource_claims {
            ...
            if let Some(previous) = files.get(&key) {
                let is_res_baseline = previous.domain == ContentDomain::ResourceBaseline
                    || crate::is_resource_baseline_path(claim.expectation().logical_path())
                    || crate::is_resource_baseline_path(previous.expectation.logical_path());
                if !is_res_baseline
                    && (previous.expectation.expected_md5() != claim.expectation().expected_md5()
                        || previous.expectation.expected_size()
                            != claim.expectation().expected_size())
                {
                    return Err(Error::Message {
                        context: "Integrity error: ",
                        detail: format!(
...
mod tests {
    use super::*;
    use griffr_hypergryph_api::{GameFileEntry, PackageInfo};
