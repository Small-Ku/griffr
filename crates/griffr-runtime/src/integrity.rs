...
use crate::task_pool::types::{ArchiveRepairGroupSpec, ArchiveRepairSession};
use crate::task_pool::{
...
use crate::{
    griffr_archives_path, is_launcher_metadata_path, is_resource_baseline_path,
    normalize_logical_path, ArtifactClaim, ArtifactProof, ContentPlan, FileIssue,
    PathOutcomeTracker, ProgressLane, ProgressSender,
};
use griffr_core::build_cdn_file_url;
use griffr_hypergryph_api::files_base_url;
...
fn deduplicate_target_tasks(tasks: Vec<Task>) -> Result<Vec<Task>> {
    let mut targets = HashMap::<String, (crate::ContentHash, Option<u64>)>::default();
    ...
    for task in tasks {
        ...
        let target = crate::artifact::physical_path_key(file.target());
...
fn remove_launcher_metadata_entries(entries: &mut Vec<griffr_hypergryph_api::GameFileEntry>) {
    entries.retain(|entry| !is_launcher_metadata_path(&entry.path));
}

fn remove_already_verified_entries(
    entries: &mut Vec<griffr_hypergryph_api::GameFileEntry>,
    install_root: &Path,
    ...
    let mut current_proofs = HashMap::<String, Vec<(&crate::ContentHash, Option<u64>)>>::default();
    current_proofs.reserve(proofs.len());
    for proof in proofs.iter().filter(|proof| proof.is_current()) {
        current_proofs
            .entry(crate::artifact::physical_path_key(proof.path()))
            .or_default()
    ...
    entries.retain(|entry| {
        let key = crate::artifact::physical_path_key(&install_root.join(&entry.path));
        let expected_hash = crate::ContentHash::from(&entry.md5);
...
fn remove_entries_owned_by_claims(
    ...
    for claim in claims {
        claims_by_path
            .entry(crate::artifact::physical_path_key(claim.path()))
            .or_insert(claim);
    }

    let mut conflict = None;
    entries.retain(|entry| {
        let key = crate::artifact::physical_path_key(&install_path.join(&entry.path));
        ...
        .map(|entry| {
            if repair {
                ...
                Task::ensure_file(FileEnsureTask {
                    ...
                    expected_hash: crate::ContentHash::from(&entry.md5),
...
mod tests {
    use super::*;
    use griffr_hypergryph_api::GameFileEntry;

    fn verify_task(path: &str, md5: &str, size: u64) -> Task {
        Task::Verify {
            path: PathBuf::from(path),
            logical_path: path.replace('\\', "/"),
            expected_hash: crate::ContentHash::Md5(md5.to_string()),
    ...
    fn current_matching_artifact_proof_removes_manifest_entry() {
        ...
        let proof = crate::ArtifactProof::from_verified_path(
            &path,
            crate::ArtifactExpectation::new("Data/game.bin", &md5, Some(payload.len() as u64)),
            crate::ArtifactSource::Archive,
        )
        .unwrap();
        let mut entries = vec![
            GameFileEntry {
    ...
    fn resource_claim_owns_overlapping_game_manifest_destination() {
        ...
        let claims = vec![ArtifactClaim::new(
            ...
            crate::ArtifactExpectation::new("file.blc", "base", Some(4)),
        )];
        let mut entries = vec![
            GameFileEntry {
    ...
    fn conflicting_resource_claim_is_rejected_before_integrity_tasks() {
        ...
        let claims = vec![ArtifactClaim::new(
            install_path.join("VFS/file.blc"),
            crate::ArtifactExpectation::new("file.blc", "resource", Some(4)),
        )];
        let mut entries = vec![GameFileEntry {
            path: "VFS/file.blc".to_string(),
            md5: "game".to_string(),
