...
use crate::{DELETE_FILES_MANIFEST_NAME, PATCH_MANIFEST_NAME};
...
impl MultiVolumeExtractor {
    ...
    pub(crate) fn discover_archive_directory(&self) -> Result<ArchiveDirectoryDiscovery> {
        ...
        let eocd_offset = eocd_offset.ok_or_else(|| Error::Message {
            context: "Extraction error: ",
            detail: "ZIP end-of-central-directory record was not found".to_string(),
        })?;
        ...
        if !zip64 && entries_on_disk_16 != total_entries_16 {
            return Err(Error::Message {
                context: "Extraction error: ",
                detail: "ZIP entry counts differ across disks; spanned archives are unsupported"
                    .to_string(),
            });
        }

        let (entry_count, central_size, relative_central_start, end_start) = if zip64 {
            let locator_start = eocd_start.checked_sub(20).ok_or_else(|| Error::Message {
                context: "Extraction error: ",
                detail: "ZIP64 locator is missing".to_string(),
            })?;
            ...
            if locator.get(0..4) != Some(&[0x50, 0x4b, 0x06, 0x07]) {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP64 locator is invalid".to_string(),
                });
            }
            if read_u32(&locator, 4)? != 0 || read_u32(&locator, 16)? != 1 {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: "Spanned ZIP64 archives are not supported; expected one logical disk"
                        .to_string(),
                });
            }
            ...
            let zip64_end = zip64_start
                ...
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP64 end-record range overflowed".to_string(),
                })?;
            if zip64_end > self.layout.total_size() {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP64 end record lies outside the archive stream".to_string(),
                });
            }
            ...
            if record.get(0..4) != Some(&[0x50, 0x4b, 0x06, 0x06]) {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP64 end record is invalid".to_string(),
                });
            }
            if read_u32(&record, 16)? != 0 || read_u32(&record, 20)? != 0 {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: "Spanned ZIP64 archives are not supported; expected raw split volumes"
                        .to_string(),
                });
            }
            ...
            (
                usize::try_from(total_entries).map_err(|_| Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP entry count exceeds usize".to_string(),
                })?,
                read_u64(&record, 40)?,
                read_u64(&record, 48)?,
                zip64_start,
        ...
        let central_start = end_start
            .checked_sub(central_size)
            .ok_or_else(|| Error::Message {
                context: "Extraction error: ",
                detail: "ZIP central directory is larger than its archive prefix".to_string(),
            })?;
        if central_start < relative_central_start {
            return Err(Error::Message {
                context: "Extraction error: ",
                detail: "ZIP central-directory offset is inconsistent with the end record"
                    .to_string(),
            });
        }
        if end_start > self.layout.total_size() {
            return Err(Error::Message {
                context: "Extraction error: ",
                detail: "ZIP central directory lies outside the archive stream".to_string(),
            });
        }
    ...
    pub(crate) fn read_archive_index(&self, directory: &ArchiveDirectory) -> Result<ArchiveIndex> {
        ...
        let central_end =
            central_start
                ...
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP central-directory range overflowed".to_string(),
                })?;
        ...
        for index in 0..directory.entry_count {
            if central.get(cursor..cursor + 4) != Some(&[0x50, 0x4b, 0x01, 0x02]) {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: format!("Invalid central-directory record at entry {index}"),
                });
            }
            ...
            let name_start =
                cursor
                    ...
                    .ok_or_else(|| Error::Message {
                        context: "Extraction error: ",
                        detail: "ZIP central-directory offset overflowed".to_string(),
                    })?;
            let name_end = name_start
                .checked_add(name_len)
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP filename range overflowed".to_string(),
                })?;
            let extra_end = name_end
                .checked_add(extra_len)
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP extra-data range overflowed".to_string(),
                })?;
            let record_end = extra_end
                .checked_add(comment_len)
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP central record range overflowed".to_string(),
                })?;
            central
                .get(name_start..name_end)
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "Truncated ZIP filename".to_string(),
                })?;
            let extra = central
                .get(name_end..extra_end)
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "Truncated ZIP extra data".to_string(),
                })?;
            central
                .get(extra_end..record_end)
                .ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: "Truncated ZIP file comment".to_string(),
                })?;
            ...
            let size = if uncompressed_32 == u32::MAX {
                zip64_uncompressed.ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: format!("ZIP64 entry {normalized} is missing its uncompressed size"),
                })?
            } else {
                u64::from(uncompressed_32)
            };
            let compressed_size = if compressed_32 == u32::MAX {
                zip64_compressed.ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: format!("ZIP64 entry {normalized} is missing its compressed size"),
                })?
            } else {
                u64::from(compressed_32)
            };
            let local_offset = if local_offset_32 == u32::MAX {
                zip64_offset.ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: format!("ZIP64 entry {normalized} is missing its local-header offset"),
                })?
            } else {
                u64::from(local_offset_32)
            };
            let disk_start = if disk_start_16 == u16::MAX {
                zip64_disk_start.ok_or_else(|| Error::Message {
                    context: "Extraction error: ",
                    detail: format!("ZIP64 entry {normalized} is missing its start-disk number"),
                })?
            } else {
            ...
            let absolute_start =
                archive_offset
                    ...
                    .ok_or_else(|| Error::Message {
                        context: "Extraction error: ",
                        detail: "ZIP local-header offset overflowed".to_string(),
                    })?;
            if absolute_start >= central_start {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: format!("ZIP entry {normalized} starts inside the central directory"),
                });
            }
        ...
        for pair in starts.windows(2) {
            if pair[0].0 == pair[1].0 {
                return Err(Error::Message {
                    context: "Extraction error: ",
                    detail: "ZIP entries share a local-header offset".to_string(),
                });
            }
        }
    ...
    pub(crate) fn read_control_payloads(
        ...
        for index in &archive_index.control_indices {
            ...
            if name == PATCH_MANIFEST_NAME {
                ...
                result.delete_manifest =
                    Some(String::from_utf8(payload).map_err(|error| Error::Message {
                        context: "Extraction error: ",
                        detail: format!("{DELETE_FILES_MANIFEST_NAME} is not UTF-8: {error}"),
                    })?);
            }
        }
        Ok(result)
