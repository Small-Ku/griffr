...
fn parse_channel(region: RegionId, value: Option<String>) -> Result<ChannelId> {
    let Some(official) = region.hypergryph_official_channel_id() else {
        return Err(Error::Message {
            context: "Configuration error: ",
            detail: format!("{region} does not expose Hypergryph launcher channel IDs"),
        });
    };
    let Some(value) = value else {
        return Ok(ChannelId::known(official));
    };
    let normalized = value.trim().to_ascii_lowercase();
    match normalized.as_str() {
        "official" => Ok(ChannelId::known(official)),
    ...
    pub fn parse(
        ...
        if region.backend() != BackendKind::Hypergryph {
            return Err(Error::Message {
                context: "Configuration error: ",
                detail: format!(
                    "{region} does not expose Hypergryph launcher channel/sub-channel IDs"
                ),
            });
        }
...
mod tests {
    ...
    fn yostar_regions_reject_explicit_channel_selectors() {
        for region in [RegionId::Kr, RegionId::En, RegionId::Jp] {
            assert!(ChannelPair::parse(region, None, None).is_err());
