...
pub(crate) fn merged_tile_count_for_flat(flat: &[FlatNode]) -> usize {
    ...
    let bounds = simulate_layout(&nodes, 900.0, 640.0);
    merge_adjacent(partition_tiles(&nodes, &bounds), &bounds, &nodes).len()
}
...
fn simulate_layout(nodes: &[SimNode], width: f64, height: f64) -> Vec<(u16, SimRect)> {
    ...
    for root in nodes.iter().copied().filter(|n| n.parent.is_none()) {
        let root_bounds = SimRect { x: 0.0, y: 0.0, w: width, h: height };
...
fn layout_children(parent_id: u16, parent_bounds: SimRect, nodes: &[SimNode], out: &mut Vec<(u16, SimRect)>) {
    let mut children: Vec<SimNode> = nodes.iter().copied().filter(|n| n.parent == Some(parent_id)).collect();
    ...
    let total_basis: f64 = children.iter().map(|n| n.flex_basis.max(1.0) + n.margin.max(0.0) * 2.0).sum();
    ...
    let axis = if parent_dir == 0 { content.w } else { content.h };
    ...
    for child in children {
        ...
        let grow_share = if total_grow > 0.0 { positive_remainder * (child.flex_grow.max(0.0) / total_grow) } else { 0.0 };
        let shrink_share = if overflow > 0.0 && total_shrink > 0.0 { overflow * (child.flex_shrink.max(0.0) / total_shrink) } else { 0.0 };
        let primary = (child.flex_basis.max(1.0) + grow_share - shrink_share).max(1.0);
        let rect = if parent_dir == 0 {
            let r = SimRect { x: cursor_x + margin, y: content.y + margin, w: (primary - margin * 2.0).max(1.0), h: (content.h - margin * 2.0).max(1.0) };
            cursor_x += primary + margin * 2.0;
            r
        } else {
            let r = SimRect { x: content.x + margin, y: cursor_y + margin, w: (content.w - margin * 2.0).max(1.0), h: (primary - margin * 2.0).max(1.0) };
...
fn partition_tiles(nodes: &[SimNode], bounds: &[(u16, SimRect)]) -> Vec<SimTile> {
    ...
    for yi in 0..ys.len().saturating_sub(1) {
        for xi in 0..xs.len().saturating_sub(1) {
            ...
            for (wid, rect) in bounds {
                if rect.contains(cx, cy) {
                    if let Some(node) = nodes.iter().find(|n| n.id == *wid) {
                        covering.push((node.z, node.id, match node.clip { 1 => true, -1 => false, _ => node.scrollable }));
                    }
                }
            }
            ...
            out.push(SimTile { bounds: SimRect { x: x0, y: y0, w: x1 - x0, h: y1 - y0 }, clipped, top: top_id });
        }
    }
    out
}

fn merge_adjacent(mut tiles: Vec<SimTile>, bounds: &[(u16, SimRect)], nodes: &[SimNode]) -> Vec<SimTile> {
    loop {
        let mut changed = false;
        'outer: for i in 0..tiles.len() {
            for j in (i + 1)..tiles.len() {
                if let Some(candidate) = merged_tile(&tiles[i], &tiles[j]) {
                    let mut others = Vec::<SimTile>::with_capacity(tiles.len().saturating_sub(2));
                    for (idx, t) in tiles.iter().enumerate() {
                        if idx != i && idx != j {
                            others.push(t.clone());
                        }
                    }
                    let safe = widgets_fit(&candidate, bounds) && no_scroll_or_clip_violation(&candidate, nodes) && !overlaps_others(candidate.bounds, &others);
                    if safe {
...
fn widgets_fit(tile: &SimTile, bounds: &[(u16, SimRect)]) -> bool {
    bounds.iter().find(|(id, _)| *id == tile.top).is_some_and(|(_, b)| {
        tile.bounds.x >= b.x
            && tile.bounds.y >= b.y
...
fn no_scroll_or_clip_violation(tile: &SimTile, nodes: &[SimNode]) -> bool {
    nodes.iter().find(|n| n.id == tile.top).is_some_and(|n| !n.scrollable && n.clip != 1)
}
