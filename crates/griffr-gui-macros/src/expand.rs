...
pub(crate) fn expand_widget_tree(root: ItemStruct, flat: Vec<FlatNode>) -> TokenStream {
    ...
    let canvas_fields = (0..9).map(|idx| {
        let field = Ident::new(&format!("tile{}", idx), ident.span());
        quote! { #field: ::winio::prelude::Child<::winio::widgets::Canvas>, }
    });
    let canvas_inits = (0..9).map(|idx| {
        let field = Ident::new(&format!("tile{}", idx), ident.span());
        quote! { #field: ::winio::widgets::Canvas = (&root), }
    });
    let canvas_struct_inits = (0..9).map(|idx| {
        let field = Ident::new(&format!("tile{}", idx), ident.span());
        quote! { #field, }
    });
    let start_arms = (0..9).map(|idx| {
        let idx = idx as usize;
        let field = Ident::new(&format!("tile{}", idx), ident.span());
        quote! {
            self.#field => {
                e => #msg_ident::Canvas(#idx, e),
            },
        }
    });
    let update_children_items = (0..9).map(|idx| {
        let idx = idx as usize;
        let field = Ident::new(&format!("tile{}", idx), ident.span());
        quote! { self.#field, }
    });
    let render_match_arms = (0..8).map(|idx| {
        let idx = idx as usize;
        let field = Ident::new(&format!("tile{}", idx), ident.span());
        quote! { #idx => &mut self.#field, }
    });
    let render_children_stmts = (0..9).map(|idx| {
        let idx = idx as usize;
        let field = Ident::new(&format!("tile{}", idx), ident.span());
    ...
    quote! {
        ...
        pub enum #event_ident {
            Redraw,
            Target(Option<::griffr_gui::ui::WidgetId>),
        }
        ...
        pub struct #comp_ident {
            ...
            pointers: [::winio::prelude::Point; 9],
        }
        ...
        pub enum #msg_ident {
            ...
            Canvas(usize, ::griffr_gui::ui::CanvasEvent),
        }
        ...
        impl ::winio::prelude::Component for #comp_ident {
            ...
            async fn init(init: Self::Init<'_>, _sender: &::winio::prelude::ComponentSender<Self>) -> ::winio::prelude::Result<Self> {
                ...
                Ok(Self {
                    ...
                    runtime,
                    pointers: [::winio::prelude::Point::new(0.0, 0.0); 9],
                })
            }
            ...
            async fn update(
                ...
                match message {
                    ...
                    #msg_ident::Resize(size) => {
                        self.root.set_loc(::winio::prelude::Point::new(0.0, 0.0))?;
                        self.root.set_size(Self::expand_size(size))?;
                        Ok(true)
                    }
                    #msg_ident::Canvas(idx, ev) => {
                        if let ::griffr_gui::ui::CanvasEvent::MouseMove(p) = &ev {
                        ...
                        let p = self
                            ...
                            .get(idx)
                            .copied()
                            .unwrap_or(::winio::prelude::Point::new(0.0, 0.0));
                    ...
                    if let Some(tile) = self.runtime.plan.tile_plan.tiles.get(idx) {
                        ...
                        if let Some(top_id) = tile.widgets.last().copied() {
                            if let Some((_, widget)) = self.widgets.iter_mut().find(|(id, _)| *id == top_id) {
                                ...
                                let local_bounds = ::griffr_gui::ui::Rect::new(0.0, 0.0, draw_w, draw_h);
        ...
        impl #comp_ident {
            fn build_widgets(runtime: &::griffr_gui::ui::UiRuntime) -> ::winio::prelude::Result<Vec<(::griffr_gui::ui::WidgetId, Box<dyn ::griffr_gui::ui::widget::Widget>)>> {
                ...
                for node in &runtime.plan.widgets {
                    let bounds = runtime
                        ...
                        .map(|(_, b)| *b)
                        .unwrap_or(::griffr_gui::ui::Rect::new(0.0, 0.0, 0.0, 0.0));
                    let clipped = runtime
                        .plan
                        .tile_plan
                        .tiles
            ...
            fn local_to_global(&self, idx: usize, p: ::winio::prelude::Point) -> ::winio::prelude::Point {
                if let Some(tile) = self.runtime.plan.tile_plan.tiles.get(idx) {
                    ::winio::prelude::Point::new(p.x + tile.bounds.x, p.y + tile.bounds.y)
                } else {
                    p
                }
            }
